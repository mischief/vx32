...
static int
tlsgen(Chan *c, char *_, Dirtab *__, int ___, int s, Dir *dp)
{
	Qid q;
	TlsRec *tr;
	char *name, *nm;
...
static void
initclearmac(Hashalg *h, int i, Secret *s, uchar *u)
{
	s->maclen = 0;
	s->mac = nomac;
}
...
static void
initRC4key(Encalg *ea, Secret *s, uchar *p, uchar *u)
{
...
static void
initclearenc(Encalg *ea, Secret *s, uchar *p, uchar *iv)
{
	s->enc = noenc;
	s->dec = noenc;
	s->block = 0;
...
static DigestState*
nomac(uchar *a, ulong b, uchar *c, ulong d, uchar *e, DigestState *f)
{
	return nil;
}
